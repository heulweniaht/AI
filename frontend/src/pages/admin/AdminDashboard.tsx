...
const dataLine = [
  { name: 'T2', DoanhThu: 4000, Kham: 2400 },
  { name: 'T3', DoanhThu: 3000, Kham: 1398 },
  { name: 'T4', DoanhThu: 2000, Kham: 9800 },
  { name: 'T5', DoanhThu: 2780, Kham: 3908 },
  { name: 'T6', DoanhThu: 1890, Kham: 4800 },
  { name: 'T7', DoanhThu: 2390, Kham: 3800 },
  { name: 'CN', DoanhThu: 3490, Kham: 4300 },
];
    ...
    return (
        <div className="p-8 md:p-12 animate-fade-in w-full max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                {[
                    { label: 'Tổng Bác sĩ', val: '450', icon: Stethoscope, color: 'bg-blue-50 text-blue-600' },
                    { label: 'Lượng BN Online', val: '12 K', icon: Users, color: 'bg-green-50 text-green-600' },
                    { label: 'Booking Tuần/KPI', val: '800', icon: Calendar, color: 'bg-purple-50 text-purple-600' },
                    { label: 'Doanh thu', val: '1.2 Tỷ', icon: DollarSign, color: 'bg-orange-50 text-orange-600' },
                ].map((stat, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
               <div className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-sm border border-gray-100 lg:col-span-2">
                  <div className="font-extrabold text-gray-900 text-2xl mb-8 flex justify-between">Biểu đồ Trực tuyến (Recharts Live) <span className="bg-green-100 text-green-700 text-sm font-bold px-3 py-1 rounded border border-green-200 self-center uppercase">Live</span></div>
               ...
               <div className="flex flex-col gap-8">
                   <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 flex-1 flex flex-col justify-center">
                      ...
                      <div className="w-full h-[220px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={dataPie} cx="50%" cy="50%" innerRadius={60} outerRadius={85} paddingAngle={5} dataKey="value" stroke="none">
                      ...
                      <div className="flex flex-wrap justify-center gap-4 mt-2">
                         {dataPie.map((entry, index) => (
                            <div key={index} className="flex items-center text-sm font-extrabold text-gray-700 shadow-sm bg-gray-50 px-2 py-1 rounded"><div className="w-3 h-3 rounded-full mr-2" style={{backgroundColor: COLORS[index]}}></div>{entry.name}</div>
                         ))}
                      </div>
                   </div>
